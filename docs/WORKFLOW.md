...
Complete end-to-end workflow documentation for the EV Charging & Battery Swap Platform.

> **Navigation:** [README](../README.md) | [Implementation Guide](IMPLEMENTATION.md) | [API Reference](API_REFERENCE.md) | [Integration Guide](../INTEGRATION_GUIDE.md)
...
├────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   📱 Mobile App / 🖥️ Web Dashboard / 🚗 IoT Devices                        │
│              │                                                              │
│              ▼                                                              │
│   ┌─────────────────────────────────────────────────────────────────────┐  │
│   │              🌐 API Gateway (Port 3000)                              │  │
│   │    /recommend  │  /ingest/*  │  /admin/*  │  /queue/*  │  /health   │  │
│   └─────────────────────────────────────────────────────────────────────┘  │
│              │                                                              │
│   ┌──────────┴──────────────────────────────────────────────────────────┐  │
│   │                                                                      │  │
...
├──────────────────────────────────────────────────────────────┤
│  1. Generate unique QR code ID (QR_abc123)                    │
│  2. Create queue entry in PostgreSQL                          │
│  3. Generate QR code PNG image                                │
│  4. Save to /qrcodes/QR_abc123.png                            │
│  5. Return QR code + image path                               │
└──────────────────────────────────────────────────────────────┘
       │
       │ User displays QR → Staff scans QR
       ▼
┌──────────────────────────────────────────────────────────────┐
│  POST /queue/verify → Update status to 'verified'            │
│  POST /queue/swap → Update status to 'swapped'               │
└──────────────────────────────────────────────────────────────┘
```

---

### 7. Battery Delivery Workflow

```
Admin creates delivery alert → Drivers notified → Driver accepts
→ Delivery in progress → Admin confirms completion
```

---

### 8. Fault Reporting Workflow

```
User/Staff reports fault → System checks severity
→ IF critical: Create ticket + notify admin
→ ELSE: Log only (no ticket)
```

---

> **Navigation:** [Back to README](../README.md) | [Implementation Guide](IMPLEMENTATION.md) | [API Reference](API_REFERENCE.md) | [Integration Guide](../INTEGRATION_GUIDE.md)
